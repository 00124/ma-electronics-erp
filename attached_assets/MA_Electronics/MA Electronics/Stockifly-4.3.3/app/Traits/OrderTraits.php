...
trait OrderTraits
{
    ...
    public function show(...$args)
    {
        ...
        if ($request->has('fields')) {
            ...
            foreach ($allOrderIteams as $allOrderIteam) {
                ...
                if ($orderType == 'grn') {
                    $itemRow['received_quantity'] = $allOrderIteam->received_quantity;
                    $itemRow['short_damaged_quantity'] = $allOrderIteam->short_damaged_quantity;
                }
                $allProducs[] = $itemRow;
    ...
    public function storing(Order $order)
    {
        ...
        if ($this->orderType == 'grn') {
            $order->order_status = 'received';
            $decodedPoId = $request->parent_order_id ? Common::getIdFromHash($request->parent_order_id) : null;
            $order->parent_order_id = (is_numeric($decodedPoId) || is_int($decodedPoId)) ? $decodedPoId : null;
            $order->supplier_invoice_number = $request->supplier_invoice_number;
            $order->delivery_challan_no = $request->delivery_challan_no;
            $order->received_by_name = $request->received_by_name;
            $order->received_by_signature = $request->received_by_signature;
            $order->received_by_date = $request->received_by_date ?: null;
            $order->checked_by_name = $request->checked_by_name;
            $order->checked_by_signature = $request->checked_by_signature;
            $order->checked_by_date = $request->checked_by_date ?: null;
            $order->approved_by_name = $request->approved_by_name;
            $order->approved_by_signature = $request->approved_by_signature;
            $order->approved_by_date = $request->approved_by_date ?: null;
        }
    ...
    public function stored(Order $order)
    {
        ...
        foreach ($allPayments as $allPayment) {
            ...
            if ($allPayment['amount'] > 0 && $allPayment['payment_mode_id'] != '') {
                ...
                } elseif ($allPayment['amount'] > $order->total) {
                    throw new ApiException('Paid amount should be less than or equal to Grand Total');
                }
                $payment->amount = $allPayment['amount'];
    ...
    public function updating(Order $order)
    {
        ...
        if ($this->orderType == 'grn') {
            $order->parent_order_id = $request->has('parent_order_id') && $request->parent_order_id
                ? Common::getIdFromHash($request->parent_order_id) : null;
            $order->supplier_invoice_number = $request->supplier_invoice_number;
            $order->delivery_challan_no = $request->delivery_challan_no;
            $order->received_by_name = $request->received_by_name;
            $order->received_by_signature = $request->received_by_signature;
