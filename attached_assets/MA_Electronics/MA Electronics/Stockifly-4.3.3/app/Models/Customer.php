...
use App\Traits\Entrust\EntrustUserTrait;
...
class Customer extends BaseModel implements AuthenticatableContract, JWTSubject
{
    ...
    /**
     * Find customer by phone (exact, normalized, digits-only). Used by POS and API.
     */
    public static function findByPhoneNumber($phone)
    {
        ...
        if (!$customer && $phone !== '') {
            ...
            if ($normalized !== '') {
                $customer = static::where('phone', $normalized)->first();
            }
            if (!$customer && $normalized !== $phone) {
                $customer = static::where('phone', '0' . $normalized)->first();
            }
        }
        if (!$customer && $phone !== '') {
            ...
            if (strlen($inputDigits) >= 10) {
                ...
                $customer = $customers->first(function ($c) use ($inputDigits) {
                    return preg_replace('/\D/', '', (string) $c->phone) === $inputDigits;
                });
            }
        }
        return $customer;
