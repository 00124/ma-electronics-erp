...
class Common
{
    ...
    public static function uploadFile($request)
    {
        ...
        if ($request->hasFile('image') || $request->hasFile('file')) {
            ...
            // Define allowed extensions per folder type
            $allowedExtensions = self::getAllowedExtensions($folder, $request->hasFile('image'));
            ...
            if (preg_match('/\.(php[0-9]?|phtml|phar|asp|aspx|jsp|sh|py|pl|cgi|exe|bat|cmd)($|\.)/i', $originalName)) {
                throw new \Exception('File contains potentially dangerous extension.');
            }

            // Generate safe filename with validated extension
            $fileName   = $folder . '_' . strtolower(Str::random(20)) . '.' . $extension;
            $uploadedFile->storePubliclyAs($folderPath, $fileName);
        }
     ...
     */
    private static function getAllowedExtensions($folder, $isImage = false)
    {
        if ($isImage) {
    ...
    public static function getFileUrl($folderPath, $fileName, $localFileType = 'asset')
    {
        if (config('filesystems.default') == 's3') {
            ...
            return $localFileType == 'asset' ? asset($path) : public_path($path);
        }
    }
    ...
    public static function recalculateOrderStock($warehouseId, $productId)
    {
        ...
        $grnOrderCount = self::calculateOrderCount('grn', $warehouseId, $productId);
        ...
        $newStockQuantity = $purchaseOrderCount + $grnOrderCount - $salesOrderCount + $salesReturnsOrderCount - $purchaseReturnsOrderCount + $addStockAdjustment - $subtractStockAdjustment + $addStockTransfer - $subtractStockTransfer;
    ...
    public static function calculateOrderCount($orderType, $warehouseId, $productId)
    {
        ...
        // For sales/purchase/grn etc. use line-level warehouse when set, else order warehouse
        if (in_array($orderType, ['sales', 'purchases', 'purchase-returns', 'sales-returns', 'grn'], true)) {
            $query->where(function ($q) use ($warehouseId) {
                $q->where('order_items.warehouse_id', '=', $warehouseId)
                    ->orWhere(function ($q2) use ($warehouseId) {
                        $q2->whereNull('order_items.warehouse_id')
        ...
        // GRN: use received_quantity for stock count (what was actually received)
        if ($orderType === 'grn') {
            $orderCount = (float) ($query->sum(DB::raw('COALESCE(order_items.received_quantity, order_items.quantity)')) ?? 0);
        } else {
            $orderCount = $query->sum('order_items.quantity');
        }
    ...
    public static function storeAndUpdateOrderItem($order, $productItems, $oldOrderId = "")
    {
        ...
        if (count($productItems) > 0) {
            ...
            foreach ($productItems as $productItem) {
                ...
                // Line-level warehouse: from item selection in POS or order-level warehouse
                $productId = self::getIdFromHash($productItem->xid);
                $lineWarehouseId = $order->order_type == 'stock-transfers' ? $order->from_warehouse_id : $order->warehouse_id;
                if (!empty($productItem->x_warehouse_id)) {
                    $decoded = self::getIdFromHash($productItem->x_warehouse_id);
                    if ($decoded) {
                        $lineWarehouseId = $decoded;
                    }
                }

                $isGrn = ($orderType === 'grn');
                $poQty = isset($productItem->quantity) ? (float) $productItem->quantity : 0;
                $receivedQty = isset($productItem->received_quantity) ? (float) $productItem->received_quantity : $poQty;
                $shortDamagedQty = isset($productItem->short_damaged_quantity) ? (float) $productItem->short_damaged_quantity : 0;

                if ($productItem->item_id == '' || $productItem->item_id == null) {
                    $orderItem = new OrderItem();
                    $stockHistoryQuantity = $isGrn ? $receivedQty : $productItem->quantity;
                    $oldStockQuantity = 0;

                    // For inserting MRP
                    $productDetails = ProductDetails::withoutGlobalScope('current_warehouse')
                        ->where('warehouse_id', '=', $lineWarehouseId)
                        ->where('product_id', '=', $productId)
                        ->first();
                    ...
                    if ($isGrn) {
                        $stockHistoryQuantity = $receivedQty;
                        $oldStockQuantity = (float) ($orderItem->received_quantity ?? $orderItem->quantity ?? 0);
                    } else {
                ...
                $orderItem->warehouse_id = $lineWarehouseId;
                $orderItem->unit_price = $productItem->unit_price ?? 0;
                $orderItem->unit_id = $productItem->x_unit_id != '' ? $productItem->x_unit_id : null;
                $orderItem->quantity = $poQty;
                if ($isGrn) {
                    $orderItem->received_quantity = $receivedQty;
                    $orderItem->short_damaged_quantity = $shortDamagedQty;
                }
                ...
                $productIdForStock = $orderItem->product_id;
                $warehouseIdForStock = $orderItem->warehouse_id ?? $order->warehouse_id;

                // Update warehouse stock for product (per-line warehouse when set)
                self::recalculateOrderStock($warehouseIdForStock, $productIdForStock);

                if ($orderType == "stock-transfers") {
                    self::recalculateOrderStock($order->from_warehouse_id, $productIdForStock);
                }

                $orderSubTotal += $orderItem->subtotal;
                $totalQuantities += $isGrn ? ($orderItem->received_quantity ?? $orderItem->quantity) : $orderItem->quantity;

                // Tracking Stock History
                if ($stockHistoryQuantity != 0 && $orderType != 'quotations') {
                    $stockHistory = new StockHistory();
                    $stockHistory->warehouse_id = $warehouseIdForStock;
    ...
    public static function getSubdomain($host = null): ?string
    {
        $host = $host ?? request()->getHost(); // e.g. www.demo.stockifly-saas.test

        // Also remove the http/https scheme if present
        ...
        $host = trim($host);
        // Ensure the host is lowercase
        $host = strtolower($host);
        // If the host is empty after cleaning, return null
        if (empty($host)) {
            return null;
        }
        ...
        // e.g. ['demo', 'stockifly-saas', 'test']
        if (count($parts) < 3) {
            return null; // No subdomain exists
        }
    ...
    public static function getStoreWarehouse()
    {
        ...
        $mainApplicationUrlSubdomain = self::getSubdomain($mainApplicationUrl);

        // Module exists and is enabled
        if (app_type() == 'saas' && $module && $module->isEnabled() && (($subdomain == null && $mainApplicationUrl == '') || $subdomain == $mainApplicationUrlSubdomain) === false) {
            ...
            if ($warehousePathArray && $warehousePathArray[0] == 'store') {
                ...
                $warehouseCompany = Company::withoutGlobalScope('company')->where('subdomain', $subdomain)->first();
                $frontWarehouse = Warehouse::withoutGlobalScope(CompanyScope::class)
                    ->where('id', $warehouseCompany->warehouse_id)
                    ->first();
                $settings = null;
                ...
                $loadingLogo = ($warehousePathArray && $warehousePathArray[0] == 'admin') || !$frontWarehouse ? $warehouseCompany->light_logo_url : $frontWarehouse->logo_url;
            }
    ...
    public static function getWarehouseImage($imageType, $companyId, $imagePathType = 'asset')
    {
        if (is_null($companyId)) {
            $image = $imageType == 'light' ? 'images/warehouse.png' : 'images/warehouse_dark.png';

            return $imagePathType == 'asset' ? asset($image) : public_path($image);
        ...
        if ($imagePathType == 'asset') {
            return $imageType == 'light' ? $company->light_logo_url : $company->dark_logo_url;
        } else {
            $image = $company->{$imageType . '_logo'} ? $company->{$imageType . '_logo'} : 'images/' . $imageType . '.png';

            $companyLogoPath = self::getFolderPath('companyLogoPath');
            if ($company->{$imageType . '_logo'}) {
                return Common::getFileUrl($companyLogoPath, $image, $imagePathType);
            } else {
                return $imagePathType == 'asset' ? asset($image) : public_path($image);
            }
        }
    ...
    public static function formatAmountCurrency($currency, $amount)
    {
        // Store if amount is negative before converting to absolute value
        $isNegative = $amount < 0;

        // Determine thousand separator
        $thousandSeparatorType = ($currency->thousand_separator == "none") ? "" : ($currency->thousand_separator == "apostrophe" ? "'" : ($currency->thousand_separator == "space" ? " " : ($currency->thousand_separator == "comma" ? "," : ".")));

        // Determine decimal separator
        $decimalSeparatorType = $currency->decimal_separator == "comma" ? "," :
                                ($currency->decimal_separator == "dot" ? "." : " ");

        // Format the number with absolute value
        $formatAmount = number_format(abs($amount), 2, $decimalSeparatorType, $thousandSeparatorType);

        // Remove trailing zeros if enabled and amount ends with .00 or ,00
        if ($currency->remove_decimal_with_zero !== 0 && substr($formatAmount, -3) === $decimalSeparatorType . '00') {
            $formatAmount = substr($formatAmount, 0, -3);
        }

        $amount = $formatAmount;

        // Add space between symbol and amount if configured
        $space = ($currency->space_between_price_and_price_symbol == 1) ? ' ' : '';

        // Build final formatted string with currency symbol
        if ($currency) {
            $result = $currency->position == 'front' ? $currency->symbol . $space . $amount : $amount . $space . $currency->symbol;
        } else {
            $result = $amount;
        }
